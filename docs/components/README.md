...
- actions and basic inputs: [Badge](badge.md), [Button](button.md), [ButtonGroup](button-group.md), [Checkbox](checkbox.md), [Switch](switch.md), and [RadioGroup](radio-group.md);
- fields and selection: [TextField](text-field.md), [NumericField](numeric-field.md), [DateField](date-field.md), [TimeField](time-field.md), [SelectField](select-field.md), [MaskedField](masked-field.md), [OtpField](otp-field.md), [DropdownSelect](dropdown-select.md), and [FileSelect](file-select.md);
- pointer/range input: [Slider](slider.md) and [RangeSlider](range-slider.md);
- overlays and feedback: [Dialog](dialog.md), [Drawer](drawer.md), [Popover](popover.md), [Overlay](overlay.md), [MessageBar](message-bar.md), [Toast](toast.md), plus shared [Containers](containers.md) infrastructure;
...
The reconciled inventory currently tracks 75 families/types across `Bluent.UI`, Charts, Diagrams, and Utilities. The main UI ledger contains 56 component families, of which 33 have source-verified canonical references; Dialog additionally retains separately recorded runtime verification.

The inventory changed from the earlier 71-item baseline after source discovery found public main-UI component directories that were absent from the old ledger, including `DropdownList`, `Link`, `TileLayout`, and `Tooltip`. `Containers` is classified as cross-component infrastructure rather than an ordinary family row.
